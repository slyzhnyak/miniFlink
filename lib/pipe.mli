...
(** Собрать [Data]-значения потока в список.
    ...
    {b ⚠ Бесконечный поток:} дренит до [None] — на бесконечном источнике
    (например Kafka-топик) зациклится навсегда. Для бесконечных потоков
    используйте {!sink} / {!iter_data} с внешним условием остановки. *)
...
(** Свернуть [Data]-значения потока. Например, подсчёт суммы:
    ...
    {b ⚠ Бесконечный поток:} дренит до конца — зациклится навсегда. *)
...
(** Свернуть поток с retract'ами в финальную таблицу записей. [by v ts]
    ...
    {b ⚠ Бесконечный поток:} дренит до [None] — на бесконечном источнике
    зациклится навсегда; применяйте к конечному потоку или срезу. *)
...
type 'out ctx = 'out Process_fn.ctx = {
  ...
  (** эмитить [Retract] значения (отзыв ранее выпущенного вывода) с тем
      ...
      раньше это делалось в обход process_keyed (см. ex07 gas_alerts). *)
...
(** {3 suppress_while — подавление одного потока алертов другим}
    ...
    Blocked-множество ПЕРСИСТЕНТНО (Managed_state, аудит 2026-07, N-1b):
    чекпоинтится по watermark и переживает рестарт — после recovery
    подавление сохраняется, без окна ложных алертов из suppressed-потока.
    В ephemeral-контексте работает как обычное in-memory множество.
    Ключ, получивший [`On] без последующего [`Off], остаётся в множестве
    (рост ограничен числом «застрявших» ключей — снимайте [`Off]). *)
val suppress_while :
