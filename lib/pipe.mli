...
(** [keyed_join (module K) streams] объединяет [streams] (все одного
    ...
    - [Retract] в input потоке клирит соответствующий slot
      в snapshot'е (slot становится [None]) и эмитит новый
      snapshot — но только если retracted значение совпадает с
      текущим в слоте (OCaml structural равенство [=]). Stale
      retract'ы (когда более новый Data уже перезаписал слот)
      игнорируются. Это поддерживает случай когда upstream
      window'ы эмитят [Retract] + [Data] для late events.
