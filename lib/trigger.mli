...
(** Применить триггер к потоку item-обновлений.
    ...
    [?ttl] (опционально): ограничение памяти per-key state. Без [ttl]
    состояние ключа хранится вечно — на неограниченном/ротирующем
    пространстве ключей это утечка. С [ttl] ключ, не обновлявшийся
    дольше [wm - ttl] И находящийся в спокойном состоянии (нет активного
    алерта и незавершённого перехода), удаляется при watermark. Ключи с
    активным алертом или pending-переходом не выселяются. Для стабильного
    пространства ключей (например ~4096 ламп minePASS) [ttl] не
    обязателен. По умолчанию — без очистки.
