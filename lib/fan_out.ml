      ...
      let rec loop () =
        if not (Queue.is_empty b.q) then begin
          ...
          (* источник упёрся в чужой полный Block-буфер, наш пуст:
             ...
             ВНИМАНИЕ для будущих ревью — это НЕ зависание/дедлок, а
             контракт Block-backpressure (см. fan_out.mli): Block-выход
             честно ждёт, пока освободят буфер другого Block-выхода.
             Поэтому при ОДНОПОТОЧНОМ чтении Block-выходы надо читать
             вперемешку; полное последовательное осушение одного
             выхода (`List.map drain streams`), пока другие Block-выходы
             никто не читает, заблокирует поток — by design. Для
             конкурентного чтения (паттерн supervisor, по одному
             выходу на поток) всё работает. Внешний bug-report R1
             ошибочно квалифицировал именно последовательное чтение как
             баг; см. test/test_fan_out_block_contract.ml, который
             показывает: sequential блокируется (ожидаемо), concurrent
             осушается. *)
