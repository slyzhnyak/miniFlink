...
type pending_timer = {
  t_fire_at : Time.t;
  t_key     : string;   (* key, сериализованный через key_to_string в of_stream *)
}
...
let of_stream
  ...
  (* Сериализация ключа в строку для строковых Hashtbl (состояние +
     ...
     Поэтому Marshal (полная сериализация значения) + 128-битный
     MD5-дайджест: детерминированно, без параметров от пользователя,
     пространство коллизий 2^128 вместо 2^30 — на любом реальном числе
     ключей коллизий нет. Состояние ключа — замкнуто-свободные данные,
     поэтому Marshal безопасен (та же причина, по которой ортогональная
     persistence сериализует состояние Marshal'ом).
