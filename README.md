...
- [docs/tutorial.tex](docs/tutorial.tex) — **сквозной getting-started
  ...
  (телеметрия датчиков), включает продвинутые операторы (process_keyed с
  ретрактами, on_silence, co_process, on_error). Лучшая точка входа для
  новых пользователей; связывает примеры ex01–ex10 единой нитью. Собрать:
  `xelatex tutorial` (кириллица через fontspec, без доп. пакетов) или
  `pdflatex tutorial` (нужен `texlive-lang-cyrillic`); [готовый
  PDF](docs/tutorial.pdf) в репо.
