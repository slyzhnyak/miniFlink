...
**Практический вывод для minePASS-деплоя:** оба пайплайна на одном
...
- [ ] **Durable-таймеры (переживающие рестарт).** Сейчас таймеры
  `process_keyed` живут в памяти оператора и сбрасываются при
  перезапуске; keyed-состояние переживает (checkpoint), поэтому работает
  паттерн пере-регистрации из снапшота (`set_event_timer_for`, покрыт
  тестом) — но он требует кода в каждом пайплайне. Полноценное решение:
  таймеры снапшотятся вместе с состоянием в checkpoint и
  восстанавливаются при recovery автоматически. Важно для
  heartbeat-задач (пропавший до краха источник должен обнаружиться
