...
- [ ] **Сквозной backpressure.** Bounded-каналы дают локальный backpressure
  dispatcher→воркеры; нет давления от медленного sink до источника.
- [ ] **DLQ с retry + экспоненциальной задержкой.** Сейчас DLQ только
  логирует / считает. Нужна политика повторов и (опц.) отправка в
  отдельный топик.
- [ ] **Idle watermark по wall-clock.** Если источник замолчал, watermark
  не двигается и окна висят. Нужен таймер, продвигающий watermark при
  тишине (требует таймера вне pull-модели — в слое source).
- [ ] **Schema evolution на деле.** `schema_default.ml` — заглушка. Нужна
  реальная миграция стейта при изменении формата.
