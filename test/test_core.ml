...
let test_window () =
  Printf.printf "\n-- Window correctness\n";
  let mk t s = { Domain.device_id="A"; speed_kmh=s; fuel_pct=80.; position={Domain.lat=55.;lon=37.}; ts=t*1000; device=None } in

  (* 3 events in one 30s window *)
  let ctx = Harness.create (fun src ->
    src
  ...
  Harness.push ctx (mk 5 60.) ~ts:5000;
  Harness.push ctx (mk 10 65.) ~ts:10000;
  Harness.push ctx (mk 20 70.) ~ts:20000;
  Harness.push_wm ctx ~ts:35000;
  let counts = Harness.run ctx in
  check_eq "3 events in 1 window" (List.hd counts) 3;

  (* Events split across 2 windows *)
  let ctx2 = Harness.create (fun src ->
    src
    |> Mf_event.with_watermarks ~latency:(seconds 3)
    |> Pipe.window (module Telemetry) (Pipe.tumbling (seconds 30))
    |> Pipe.aggregate (fun k _ev -> k)) in
  Harness.push ctx2 (mk 5 60.) ~ts:5000;
  Harness.push ctx2 (mk 35 65.) ~ts:35000;
  Harness.push_wm ctx2 ~ts:65000;
  let wins = Harness.run ctx2 in
...
let test_dedup () =
  Printf.printf "\n-- Dedup cooldown\n";
  let mk rule ts =
    { Domain.id=""; device_id="A"; rule; severity=Warning; message=""; ts } in
  let ctx = Harness.create (fun src ->
    src |> Pipe.dedup (module Alert) ~rule:(fun a -> a.rule) ~cooldown:(minutes 5)) in
  Harness.push ctx (mk "speed" 1000)   ~ts:1000;    (* passes *)
  Harness.push ctx (mk "speed" 61000)  ~ts:61000;   (* suppressed: 1 min < 5 min *)
  Harness.push ctx (mk "speed" 361000) ~ts:361000;  (* passes: 6 min > 5 min *)
  Harness.push ctx (mk "fuel"  2000)   ~ts:2000;    (* passes: different rule *)
  let r = Harness.run ctx in
...
let test_enrich () =
  ...
  Harness.push ctx { Domain.device_id="A"; speed_kmh=60.; fuel_pct=80.; position={Domain.lat=55.;lon=37.}; ts=1000; device=None } ~ts:1000;
  Harness.push ctx { Domain.device_id="B"; speed_kmh=70.; fuel_pct=70.; position={Domain.lat=55.;lon=37.}; ts=2000; device=None } ~ts:2000;
  Harness.push ctx { Domain.device_id="X"; speed_kmh=50.; fuel_pct=90.; position={Domain.lat=55.;lon=37.}; ts=3000; device=None } ~ts:3000;  (* unknown device *)
  let r = Harness.run ctx in
...
let test_full_pipeline () =
  ...
  List.iter (fun (id,t,s,f) ->
    Harness.push ctx { Domain.device_id=id; speed_kmh=s; fuel_pct=f; position={Domain.lat=55.;lon=37.}; ts=t*1000; device=None } ~ts:(t*1000)
  ) [
    "A",  0,  60., 80.;
    "B",  2, 100., 60.;
    "A",  5,  65., 79.;
  ...
  Harness.push_wm ctx ~ts:65000;
  let alerts = Harness.run ctx in
