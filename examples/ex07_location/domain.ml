...
(** Результат локации в одном окне: шахтёр, конец окна, top-2 маяков с
    ...
    [loc_position] — приближение через линейную интерполяцию между двумя
    сильнейшими маяками (см. {!Pipelines.interpolate_position}). Это
    демо-уровень: для production-точности нужна трилатерация на 3+
    маяках. [None] если меньше 2 маяков в окне. *)
type location = {
  loc_lamp     : string;
  loc_wend     : Time.t;
