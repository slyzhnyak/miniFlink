...
let raw_events = [
  { r_bus="B1"; r_depot="north"; r_speed=42.; r_charge=80.;  r_ts=seconds 1 };
  { r_bus="B2"; r_depot="north"; r_speed=38.; r_charge=15.;  r_ts=seconds 2 };
  { r_bus="B3"; r_depot="south"; r_speed=55.; r_charge=95.;  r_ts=seconds 3 };
  { r_bus="B1"; r_depot="north"; r_speed=40.; r_charge=(-1.); r_ts=seconds 4 }; (* битый заряд *)
  { r_bus="B4"; r_depot="south"; r_speed=60.; r_charge=8.;   r_ts=seconds 5 };
  { r_bus="B2"; r_depot="north"; r_speed=45.; r_charge=12.;  r_ts=seconds 6 };
  { r_bus="B5"; r_depot="south"; r_speed=nan;  r_charge=70.; r_ts=seconds 7 }; (* битая скорость *)
  { r_bus="B3"; r_depot="south"; r_speed=52.; r_charge=90.;  r_ts=seconds 8 };
]
...
let pipeline source =
  ...
  |> Pipe.window_agg (module ByDepot) (Pipe.tumbling (seconds 10))
...
let () =
  ...
  (* (A) Агрегация с изоляцией битых событий *)
  Printf.printf "[A] window_agg + safe_map (битые показания пропускаются)\n";
  Mf_event.of_list ~ts:(fun r -> r.r_ts) raw_events
  |> pipeline
  |> Stream.to_list
  |> List.iter (function
       | Mf_event.Data ((depot, ((avg_speed, min_charge), low_count)), wend) ->
         ...
         Printf.printf "  окно[..%ds] депо %-6s: ср.скорость %-10s мин.заряд %-5s  низкий заряд: %d\n"
           (wend/1000) depot avg mc low_count
       | _ -> ());
  Printf.printf "  битых показаний пропущено: %d\n\n" !dropped;
