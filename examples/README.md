...
| [`ex07_location.ml`](ex07_location.ml) | **Локация шахтёра по маякам связи**: пакеты фонарей (до 5 слышимых маяков с RSSI, шаг 15с, маяки слышны только на своём горизонте) → `flat_map` разворот → **median RSSI** (`Agg.median` гасит шум) по ключу шахтёр×маяк в **скользящем окне** минута/5с (одно событие в 12 окон — локация обновляется каждые 5с при пакетах раз в 15с) → top-2 маяков в **каждом окне** + координаты. **Два алерта** через один event-time таймер (две независимые ловушки на один ключ небезопасны: `(key,time)` Set сливает совпадения): «нет пакетов >2 мин» и «не слышит маяки >5 мин» с дедупликацией. Опоздавшие пакеты → `allowed_lateness` → ретракции пересчитывают закрытые окна; один маяк → предупреждение для трилатерации. |
