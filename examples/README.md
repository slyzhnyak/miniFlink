...
| [`ex07_location/`](ex07_location/) | **Локация шахтёра по маякам связи** — структурирован как реальный сервис. `Domain` (типы, пороги), `Mock_source` (мок «Kafka-source» с базовым потоком и слоем «дрожания канала»), `Pipelines` (ядро: `median_rssi` + `connectivity_alerts` с тремя слоями FSM), `Mock_sink` (рендеринг как push в Kafka-sink), тонкая топология в `ex07_location.ml`. Ядро вынесено в библиотеку `ex07_location_lib` и переиспользуется бенчмарками и regression-тестами. В прода-конфигурации `Mock_source`/`Mock_sink` заменяются на реальные `Kafka_source`/`Kafka_sink`, остальное без изменений. Покрывает: 4 алерта диагностики (нет пакетов / не слышит маяки / не двигается / низкое напряжение с гистерезисом+debounce) + импульс SOS, скользящие окна 60с/5с с обновлением раз в 5с при пакетах 15с, dedup для дублей, `allowed_lateness` для опоздавших с ретракциями. |
