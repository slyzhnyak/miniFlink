...
| [`ex05_fleet.ml`](ex05_fleet.ml) | **Сервис целиком** (мониторинг флота электробусов): как операторы собираются в работающий сервис — `safe_map` (битые показания не роняют сервис), `window_agg` + `Agg` (метрики по депо с алертами о низком заряде), `allowed_lateness` (терпит опоздавшую телеметрию), **exactly-once + recovery** (накопительная статистика переживает перезапуск), `retry` (устойчивая публикация сводки), `health`. Не демонстрация операторов, а код сервиса. |
