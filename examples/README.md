...
| [`ex07_location/`](ex07_location/) | **Локация шахтёра по маякам связи** — структурирован как реальный сервис. `Domain` (типы, пороги), `Mock_source` (мок «Kafka-source»: модуль `Default` для базового сценария — 6 шахтёров с late+dups, и параметризуемый `Large_mine` для бенчмарка — 16×64 биконов + 16×256 шахтёров с процентным распределением алертов), `Pipelines` (ядро: `median_rssi` + `connectivity_alerts` с тремя слоями FSM), `Mock_sink` (рендеринг как push в Kafka-sink, sink принимает опциональный справочник координат), тонкая топология в `ex07_location.ml`. Ядро вынесено в библиотеку `ex07_location_lib` и переиспользуется `bench/bench_ex07.ml` (полная конфигурация: ~950K пакетов, ~2400 алертов всех типов, ~180K ретракций) и regression-тестами. В прода-конфигурации `Mock_source`/`Mock_sink` заменяются на реальные `Kafka_source`/`Kafka_sink`, остальное без изменений. Покрывает: 4 алерта диагностики (нет пакетов / не слышит маяки / не двигается / низкое напряжение с гистерезисом+debounce) + импульс SOS, скользящие окна 60с/5с с обновлением раз в 5с при пакетах 15с, dedup для дублей, `allowed_lateness` для опоздавших с ретракциями. |
