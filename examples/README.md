...
| [`ex07_location.ml`](ex07_location.ml) | **Локация шахтёра по маякам** (ядро minePASS): пакеты фонарей (топ-5 слышимых маяков с RSSI) → `flat_map` разворот → **median RSSI** (`Agg.median` гасит выбросы) по ключу шахтёр×маяк в **скользящем окне** минута/5с → top-2 узла + координаты. Плюс граничные случаи: **алерт пропадания связи** («не слышит узлы >5 мин», `process_keyed` + event-time таймер; пустые пакеты таймер НЕ сбрасывают), **опоздавшие пакеты** (`allowed_lateness` → ретракции пересчитывают закрытые окна, счётчик в выводе), **один слышимый узел** (честное предупреждение: для трилатерации мало). |
