...
**Итого:** 120 тестов, 11 примеров, end-to-end атомарная коррекция
...
## Последующий аудит (июнь 2026, раунды 4-5)

Phase 1-3 заложили механику Update, но систематическое покрытие
семантики (раунд 4) и работа над реалистичностью примеров (раунд 5)
вскрыли, что «conservative handling» в нескольких операторах был
тонко неверным. Все исправлено с регрессионными тестами:

- **filter** давал молча неверный результат на asymmetric Update
  (предикат по-разному реагирует на old/new). Раньше Update
  пропускался/отбрасывался целиком по `p(new)`. Стало: полный разбор
  `(p old, p new)` — `(F,T)→Data(new)` (появление), `(T,F)→Retract(old)`
  (исчезновение), `(T,T)→Update`, `(F,F)→drop`; Retract проходит
  только если `p(value)`. См. inspection-2026-06-round4-coverage.md.

- **window_agg** эмитил noop `Update{r→r}` на каждое late-событие,
  не менявшее результат (sum+0, median тем же значением). Стало:
  подавление после `finish` (сравнение результата, не аккумулятора —
  у median acc меняется при неизменном результате).

- **group_by** молча терял late data: его mutable-`Hashtbl`
  аккумулятор делал `old` и `new` в `Update{old, new}` одним
  объектом → коррекция выглядела noop и подавлялась. Стало: immutable
  `Map` (персистентные снимки, дёшево через структурное разделение).
  См. inspection-2026-06-round5-examples.md, load-test-2026-06.md.

- **keyed_join** и **trigger** проверены покрытием — корректны
  (atomic Update без None-flicker; коррекция через порог даёт
  recovery/alert).

Итог: матрица «оператор × тип события» в
inspection-2026-06-round4-coverage.md фиксирует, как каждый оператор
обрабатывает Data/Watermark/Retract/Update. Примеры (ex05/ex07/ex09/
ex10) демонстрируют реальные late-data коррекции, а не фиктивные.
