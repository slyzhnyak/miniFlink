...
(retry в mesh при потерянном ACK, ретрансляция ВОЛС-коммутатором,
at-least-once Kafka), мы дедуплицируем по бизнес-ключу
\emph{пакета} (lamp + event-time), а не по байтовому содержимому
сетевого сообщения --- которое пограничный коммутатор вполне мог
обогатить полем \texttt{received\_at} перед ретрансляцией.
...
большим числом hop'ов в mesh или частыми перестроениями маршрута
может потребоваться 5--10с.
...
| Some level, Some old ->
  ...
  else (* атомарный Update: old -> new одним событием *)
\end{lstlisting}
...
(\emph{sticky} семантика, защита от спама). Иначе --- атомарный
\texttt{Update old new}: одно событие, несущее и старое, и новое
значение. В отличие от пары \texttt{Retract+Data}, downstream видит
коррекцию за один шаг, без промежуточного состояния «алерт исчез, нового
ещё нет» (которое в snapshot-потребителе дало бы мигание). Случай~2 выше
(алерт \emph{снят}) остаётся парой \texttt{Retract + Gas\_resolved}: там
замены нет, есть исчезновение плюс отдельное событие-новость, и для этого
\texttt{Update} не подходит. Так ex07 показывает оба: \texttt{Update} для
замены, \texttt{Retract} для исчезновения.
