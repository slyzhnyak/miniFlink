...
Исторически gas_alerts был вручную реализованным temporal-left-join
трёх потоков (union-тег + свой Hashtbl + Queue + pull-цикл). В
DSL-раунде 2026-07 добавлен `Pipe.co_process2/3` — co-обработка
разнотипных потоков на общем per-key состоянии — и газовый пайплайн
переписан декларативно на три доменных обработчика (см.
`docs/expressiveness.md`, раздел 4). Это ровно тот co-partitioned join,
которого не хватало; абстракция теперь есть.

В Section 3.3 статьи упоминается **«hand-rolled stateful stream»** для
gas_alerts — это описание прежней реализации, до co_process.

**Сложность (была).** Средняя. Семантика join'а с retract-ами +
watermarks не тривиальна — поэтому оператор и появился в библиотеке
(один раз, оттестированный), а не в каждом пайплайне.
